...
namespace escapes
{
  ...
  public:
    ...
    ast::VarDec* get_var(misc::symbol sym);

    void put(ast::VarDec* var);

  private:
    misc::scoped_map<misc::symbol, ast::VarDec*> var_;
  };

  inline ast::VarDec* EscapesVisitor::get_var(misc::symbol sym)
  {
    auto cur = var_.get_scopes();
    ast::VarDec* r = nullptr;
    for (auto i = cur.rbegin() + 1; i != cur.rend() && r == nullptr; i++)
      r = (*i)[sym];
    return r;
  }
