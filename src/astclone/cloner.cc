...
namespace astclone
{
  ...
  void Cloner::operator()(const ast::ArrayExp& e)
  {
    ...
    NameTy* type = recurse(e.get_type());
    Exp* lhs = recurse(e.get_lhs());
    Exp* rhs = recurse(e.get_rhs());
    result_ = new ArrayExp(location, type, lhs, rhs);
  }
  ...
  {
    const Location& location = e.location_get();
    Var* val = recurse(e.get_var());
    Exp* exp = recurse(e.get_exp());
    result_ = new AssignExp(location, val, exp);
  }
  ...
  void Cloner::operator()(const ast::CallExp& e)
  {
    ...
    exps_type* exp = new exps_type();
    for (auto a : e.get_seq())
      exp->push_back(recurse(*a));
    result_ = new CallExp(location, name, exp);
  }
  ...
  {
    const Location& location = e.location_get();
    Var* var = recurse(e.get_var());
    result_ = new FieldVar(location, var, e.get_name());
  }
  ...
  void Cloner::operator()(const ast::IfExp& e)
  {
    const Location& location = e.location_get();
    Exp* exp = recurse(e.get_exp());
    Exp* then = recurse(e.get_then());
    Exp* else_ = recurse(e.get_else());
    result_ = new IfExp(location, exp, then, else_);
  }
  ...
  {
    const Location& location = e.location_get();
    ChunkList* chunk = recurse(e.get_chunk());
    Exp* in = recurse(e.get_exp());
    result_ = new LetExp(location, chunk, in);
  }

  void Cloner::operator()(const ast::MethodCallExp& e) { throw "unreachable"; }
  ...
  void Cloner::operator()(const ast::ObjectExp& e) { throw "unreachable"; }
  ...
  {
    const Location& location = e.location_get();
    NameTy* name = recurse(e.get_type());
    fieldinits_type* field = new fieldinits_type();
    for (auto a : e.get_field())
      field->push_back(recurse(*a));
    result_ = new RecordExp(location, name, field);
  }

  void Cloner::operator()(const ast::RecordTy& e)
  {
    const Location& location = e.location_get();
    fields_type* field = new fields_type();
    for (auto a : e.get_field())
      field->push_back(recurse(*a));

    result_ = new RecordTy(location, field);
  }
  ...
  void Cloner::operator()(const ast::SeqExp& e)
  {
    ...
    result_ = new SeqExp(location, exp);
  }
  ...
  {
    const Location& location = e.location_get();
    std::string s = e.get_var();
    auto exp = new StringExp(location, s);
    result_ = exp;
  }
