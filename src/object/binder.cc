...
￼
  ...
  void Binder::operator()(ast::SimpleVar& e)
  {
    if (within_method_dec_)
      {
        auto res = v_scope_.get(e.name_get());
        if (!res)
          {
  ...
  {
    auto res = t_scope_.get(e.name_get());
  ...
  template <class D> void Binder::chunk_visit(ast::Chunk<D>& e)
  {
    ...
    for (auto curs = chunk.begin(); curs != chunk.end(); curs++)
      {
        auto a = (*curs)->name_get().get();
        for (auto sub_curs = chunk.begin(); sub_curs != curs; sub_curs++)
          {
            auto b = (*sub_curs)->name_get().get();
            if (a == b)
              {
                redefinition(**sub_curs, **curs);
                break;
              }
          }
        visit_dec_header<D>(**curs);
      }
    for (auto& curs : e.decs_get())
      visit_dec_body(*curs);
  }
